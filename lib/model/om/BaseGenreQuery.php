 ...
 *
 * @method     GenreQuery leftJoinShow($relationAlias = null) Adds a LEFT JOIN clause to the query using the Show relation
 * @method     GenreQuery rightJoinShow($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Show relation
 * @method     GenreQuery innerJoinShow($relationAlias = null) Adds a INNER JOIN clause to the query using the Show relation
 *
...
abstract class BaseGenreQuery extends ModelCriteria
{
	...
	{
		$sql = 'SELECT `ID`, `NAME` FROM `genre` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new Genre();
			$obj->hydrate($row);
			GenrePeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
	 ...
	 *
	 * @param     Show $show  the related object to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    GenreQuery The current query, for fluid interface
	 */
	public function filterByShow($show, $comparison = null)
	{
		if ($show instanceof Show) {
			return $this
				->addUsingAlias(GenrePeer::ID, $show->getGenreId(), $comparison);
		} elseif ($show instanceof PropelCollection) {
			return $this
				->useShowQuery()
				->filterByPrimaryKeys($show->getPrimaryKeys())
				->endUse();
		} else {
